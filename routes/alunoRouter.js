...
alunoRouter.get("/api/atividades/todas/:RA", verificarJWTAluno, async (req, res, next) => {
...
alunoRouter.get("/api/atividades/negadas/:RA", verificarJWTAluno, async (req, res, next) => {
  const RA = req.params.RA;
  atividadeSchema.find(
    {
      RA: RA,
...
alunoRouter.get("/api/atividade/:ID", verificarJWTAluno, async (req, res, next) => {
...
alunoRouter.post("/api/atividade", verificarJWTAluno, async (req, res, next) => {
...
alunoRouter.patch("/api/atividade", verificarJWTAluno, async (req, res, next) => {
  atividadeSchema.findByIdAndUpdate(req.body._id, { $set: req.body }, (err) => {
    if (!err) {
...
alunoRouter.delete("/api/atividade/:ID", verificarJWTAluno, async (req, res, next) => {
...
async function verificarJWTAluno(req, res, next) {
  ...
  jwt.verify(token, publicKey, { algorithm: ["RS256"] }, (err, decoded) => {
    if (err) {
      ...
      if (decoded.acesso === 'user' || decoded.acesso === 'professor' || decoded.acesso === 'adm') {
        next();
      } else {
